...
useEffect(() => {
    ...
    labsdata.forEach(lab => {
        // Generate stable ratings based on lab ID
        const seed = lab.lab_id.charCodeAt(0);
        ratings[lab.lab_id] = {
            rating: ((seed % 10) + 39) / 10,
            reviews: (seed % 51) + 75
        };
    });
									...
									<div>
										...
										<div className="flex items-center mt-1">
											<div className="flex items-center bg-green-100 px-2 py-1 rounded">
												<span className="text-green-700 font-bold mr-1">{labRatings[lab.lab_id]?.rating?.toFixed(1) || '4.0'}</span>
												<FaStar className="text-green-700 w-4 h-4" />
											</div>
											<span className="text-gray-500 text-sm ml-2">({labRatings[lab.lab_id]?.reviews || 100} reviews)</span>
										</div>
									</div>
